...
export const LuxuryBanner = React.memo(
    ({
       slides,
       autoplay = true,
       autoplaySpeed = 5000,
      ...
      // Показываем постер в первую секунду, затем загружаем видео
      useEffect(() => {
      ...
      useEffect(() => {
        if (isPageLoaded && slides[currentSlide].type === "video" && videoRefs.current[currentSlide]) {
          videoRefs.current[currentSlide]?.play();
        }
      }, [currentSlide, slides, isPageLoaded]);
                  ...
                  {slide.alignment === "split" ? (
                      ...
                      <div className="banner-content">
                        <div className={`banner-container content-${slide.alignment || "left"}`}>
                          {slide.category && (
                              <div className="banner-category">{slide.category}</div>
                          )}
                          <h2 className="banner-title">{slide.title}</h2>
                          {slide.subtitle && (
                              <h3 className="banner-subtitle">{slide.subtitle}</h3>
                          )}
                          {slide.description && (
